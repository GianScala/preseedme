  ...
  switch (rank) {
    case 1:
      return { 
        emoji: "🏆", 
        label: "1st Place", 
        badgeClass: "bg-yellow-500/10 border-yellow-500/50 text-yellow-200 shadow-[0_0_15px_rgba(234,179,8,0.2)]" 
      };
    case 2:
      return { 
        emoji: "🥈", 
        label: "2nd Place", 
        badgeClass: "bg-neutral-400/10 border-neutral-400/50 text-neutral-200 shadow-[0_0_15px_rgba(163,163,163,0.1)]" 
      };
    case 3:
      return { 
        emoji: "🥉", 
        label: "3rd Place", 
        badgeClass: "bg-amber-700/10 border-amber-700/50 text-amber-200 shadow-[0_0_15px_rgba(180,83,9,0.2)]" 
      };
    default:
      return { 
        emoji: "⭐", 
...
const MetricBadge = ({
  ...
  <div className="flex items-center gap-1.5 px-2 py-1 rounded-md bg-white/5 border border-white/5 shadow-sm">
    <span className={`w-1.5 h-1.5 rounded-full shadow-[0_0_5px_currentColor] ${dotColor}`} />
...
export default function PublicWeeklyWinner({
  ...
  const handleLikeClick = (event: MouseEvent<HTMLButtonElement>) => {
    // Stop propagation so we don't trigger the card link
  ...
  return (
    <div className="group relative flex flex-col w-[300px] sm:w-[340px] h-full rounded-2xl border border-white/5 bg-neutral-900/40 backdrop-blur-md overflow-hidden transition-all duration-500 ease-out hover:border-white/10 hover:shadow-2xl hover:-translate-y-1">
      ...
      <Link href={`/ideas/${id}`} className="absolute inset-0 z-10 focus:outline-none" aria-label={`View ${title}`}>
      ...
      {/* 3. RANK BADGE */}
      <div className={`absolute top-4 left-4 z-20 inline-flex items-center gap-1.5 px-3 py-1 rounded-full backdrop-blur-md border ${rankConfig.badgeClass}`}>
        <span className="text-sm shadow-sm">{rankConfig.emoji}</span>
        <span className="text-[11px] font-bold tracking-wide uppercase">{rankConfig.label}</span>
      </div>

      {/* 4. MAIN CONTENT */}
      {/* Pointer events none allows clicks to pass through to the Link layer, 
          but we re-enable it for the button later. */}
      <div className="flex-1 flex flex-col p-5 sm:p-6 pt-14 pointer-events-none">
        ...
        {hasMetrics && (
          <div className="flex flex-wrap items-center gap-2 mt-4 pt-4 border-t border-white/5">
            {mrrLabel && <MetricBadge value={mrrLabel} label="MRR" dotColor="bg-emerald-500 text-emerald-500" />}
            {usersLabel && <MetricBadge value={usersLabel} label="Users" dotColor="bg-[var(--brand)] text-[var(--brand)]" />}
          </div>
        )}
      </div>

      {/* 5. FOOTER */}
      {/* Note: pointer-events-none is inherited, so we must enable pointer-events-auto on the button */}
      <div className="relative z-20 px-5 sm:px-6 py-3 bg-white/[0.02] border-t border-white/5 flex items-center justify-between pointer-events-none group-hover:bg-white/[0.04] transition-colors">
        ...
        <div className="flex items-center gap-2.5">
          <div className="w-5 h-5 rounded-full bg-gradient-to-br from-[var(--brand)] to-[var(--brand-dark)] flex items-center justify-center text-[9px] font-bold text-black shadow-[0_0_10px_rgba(33,221,192,0.2)]">
        ...
        >
           {loadingLike ? (
             ...
             <HeartIcon className={`w-3.5 h-3.5 ${isLiked ? "fill-current animate-pulse-fast" : ""}`} />
           )}
           <span>{likeCount}</span>
        </button>
      </div>
