...
export default function EditIdeaPage() {
      ...
      try {
        ...
        if (data.targetMarket) {
          if (Array.isArray(data.targetMarket)) {
            ...
            typeof data.targetMarket === "string" &&
  ...
  const isDemoEmpty =
    !formData.demoVideoUrl.trim() && !formData.thumbnailPreview;
  ...
  const isBusinessEmpty =
    ...
    formData.targetMarket.length === 0;
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const data: any = {
        ...
        targetMarket: formData.targetMarket.length
          ? formData.targetMarket
          : null,
        sectors: formData.sectors.length ? formData.sectors : null,
        targetAudiences: formData.targetAudiences.length
          ? formData.targetAudiences
          : null,
        targetDemographics: formData.targetDemographics.length
          ? formData.targetDemographics
          : null,
        revenueModels: formData.revenueModels.length
          ? formData.revenueModels
          : null,
        ...
        // 📦 Deliverables (NEW)
        deliverablesOverview:
          formData.deliverablesOverview.trim() || null,
        deliverablesMilestones:
          formData.deliverablesMilestones.trim() || null,
      };
      ...
      data.totalRevenueSinceInception =
        totalRevenueNum !== undefined ? totalRevenueNum : null;
      data.monthlyRecurringRevenue =
        mrrNum !== undefined ? mrrNum : null;
      ...
      if (formData.isFundraising) {
        ...
        data.fundraisingGoal =
          fundraisingGoalNum !== undefined ? fundraisingGoalNum : null;
        data.fundraisingRaisedSoFar =
          fundraisingRaisedNum !== undefined ? fundraisingRaisedNum : null;
        data.fundraisingMinCheckSize =
          fundraisingMinCheckNum !== undefined ? fundraisingMinCheckNum : null;
      } else {
  ...
  return (
    <div className="space-y-8 pb-16">
      ...
      <header className="space-y-3">
        ...
        <div className="flex items-start gap-2 p-4 rounded-xl border border-amber-500/30 bg-amber-500/5">
          ...
          <p className="text-sm text-amber-300">
            Your changes will be saved and immediately visible to everyone
            viewing your idea.
          </p>
        </div>
      </header>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        {/* Deliverables – below funding request */}
        <DeliverablesFormSection
          ...
          showEmptyWarning={isDeliverablesEmpty}
        />
